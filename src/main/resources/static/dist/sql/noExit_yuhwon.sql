...
BEGIN
    ...
    EXCEPTION
        WHEN ERR_INVALID_USER THEN
            ROLLBACK;
            RAISE_APPLICATION_ERROR(-20999, '회원 정보가 유효하지 않습니다.');
        WHEN ERR_INVALID_RESERV THEN
            ROLLBACK;
            RAISE_APPLICATION_ERROR(-20998, '예약 정보가 유효하지 않습니다.');
        WHEN ERR_INVALID_CANCEL THEN
            ROLLBACK;
            RAISE_APPLICATION_ERROR(-20997, '예약 취소가 불가능한 시간입니다.');
        WHEN ERR_INVALID_RIGHT THEN
            ROLLBACK;
            RAISE_APPLICATION_ERROR(-20996, '예약 취소 권한이 없습니다.');
END;
/ 
