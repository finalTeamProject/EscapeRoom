...
public class UserController {
	...
	public void withdraw(@RequestParam(name = "password") String password,
	                     HttpSession session, HttpServletResponse response) throws IOException {
		...
		boolean result = service.withdraw(loginUser.getUserId(), password, session);
		if (result) {
			response.getWriter().print("{\"status\":\"success\"}");
		} else {
			response.getWriter().print("{\"status\":\"fail\",\"message\":\"비밀번호가 올바르지 않습니다.\"}");
		}
	}
